...
use App\Services\RenstraSubKegiatanServices;
use Illuminate\Database\Eloquent\Collection;
use Illuminate\Http\Request;
use Yajra\DataTables\DataTables;

class RenstraSubKegiatanController extends ApiController
{
    public function __construct(RenstraSubKegiatanServices $services)
    {
    ...
    public function dataTable()
    {
        ...
        ->editColumn('total_pagu_renstra', function ($model) {
            ...
            ->addColumn('action', function ($model) use ($tahun) {
                // if ($tahun == date('Y'))
                    return '<a href="' . route('renstra.sub-kegiatan.edit', [$model->uuid]) . '" class="btn btn-warning btn-sm font-weight-bolder m-1" ><span class="fa fa-edit"></span> Edit</a>
    ...
    public function getSubKegiatanAll()
    {
        ...
        $sub_kegiatan_dpa = $unit_kerja->RenstraSubKegiatan()->where(function ($q) {
            if (request()->has('uuid'))
