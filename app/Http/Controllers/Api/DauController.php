...
use App\Services\DauServices;
use Illuminate\Database\Eloquent\Collection;
use Illuminate\Http\Request;
use Yajra\DataTables\DataTables;

class DauController extends ApiController
{
    public function __construct(DauServices $services)
    {
    ...
    public function dataTable()
    {
        ...
        $data2 = Dpa::query()
            ...
            ->select('dpa.*',
                ...
                'sumber_dana_dpa.uuid as uuid_sd', 'sumber_dana_dpa.nilai_pagu as nilai_pagu_dau')
            ...
            ->where('sumber_dana_dpa.sumber_dana', 'NOT LIKE', '%DAK%')
            ->where('sumber_dana_dpa.jenis_belanja', 'Belanja Operasi')
            ->whereRaw('id_sub_kegiatan IN (1236, 1178, 708, 1361, 1909, 1761)')
        ...
        return DataTables::of($data2)
            ...
            ->editColumn('nilai_pagu_dpa', function ($model) {
                return 'Rp.' . number_format($model->nilai_pagu_dau, 2, ',', '.');
            })
            ...
            //     $anggaran = $model->PaketDau()->where('id_sumber_dana_dpa', $model->id_sumber_dana_dpa)->sum('anggaran_dau');
            //     $pendampingan = $model->PaketDau()->where('id_sumber_dana_dpa', $model->id_sumber_dana_dpa)->sum('pendampingan');
            ...
            //     $pendampingan = $model->PaketDau()->where('id_sumber_dana_dpa', $model->id_sumber_dana_dpa)->sum('pendampingan');
            //     $target = $anggaran + $pendampingan;
            //     if ($model->nilai_pagu_dau == $target) {
            ...
            ->addColumn('action', function ($model) use ($tahun) {
                // if ($tahun == date('Y')) {
                    ...
                    if ($disabled2 == '') {
                        $btn_text = 'Set Paket';
                        $url = cekJadwal('Kegiatan', 'Sub Kegiatan DPA Pokok') ? route('kegiatan.dau.perencanaan', $model->uuid) . '?uuid_sd=' . $model->uuid_sd : '#';
    ...
    public function dataTableRealisasi()
    {
        ...
        return DataTables::of($data2)
            ...
            // ->editColumn('status_input', function ($model) {
            //     $anggaran = $model->PaketDau()->sum('anggaran_dau');
            //     $pendampingan = $model->PaketDau()->sum('pendampingan');
            //     $target = $anggaran + $pendampingan;
            //     if ($model->nilai_pagu_dau <= $target) {
            //         $status = '<span class="label label-lg label-success label-inline m-1">Selesai</span>';
            //     } else {
            ...
            ->addColumn('action', function ($model) use ($tahun) {
                // if ($tahun == date('Y')) {
                    ...
                    $btn_text = 'Realisasi';
                    $url = route('atur.realisasi-dau', $model->uuid) . '?uuid_sd=' . $model->uuid_sd;
                    $disabled = '';
                    $additional = '';
    ...
    public function dataTableRealisasi2()
    {
        ...
        $data2 = SumberDanaDpa::query()
            ->join('paket_dau', 'paket_dau.id_sumber_dana_dpa', '=', 'sumber_dana_dpa.id')
            ...
            ->join('bidang_urusan', 'program.id_bidang_urusan', '=', 'bidang_urusan.id')
            ->join('urusan', 'bidang_urusan.id_urusan', '=', 'urusan.id')
            ->select('dpa.*', 'paket_dau.nama_paket',
                'sub_kegiatan.kode_sub_kegiatan',
                'sub_kegiatan.nama_sub_kegiatan',
                'kegiatan.kode_kegiatan',
                'kegiatan.nama_kegiatan',
    ...
    public function updateRealisasi(Request $request, $uuid)
    {
        ...
        $paket_daus = PaketDau::select('id', 'id_sumber_dana_dpa','kecamatan','desa')->where('id_sumber_dana_dpa', $sumber_dana->id)->get();
//        print_r($paket_daus);
        foreach ($keuangan as $index => $value) {
            foreach ($paket_daus as $paket_dau) {
                $id_sumber_dana_dpa = $sumber_dana->id;
                $paket_dau->update([
                    'kecamatan' => $request->kecamatan[$index][$paket_dau->id],
                    'desa' => $request->desa[$index][$paket_dau->id],
                    'kontrak' => $request->kontrak[$index][$paket_dau->id],
                    'swakelola' => $request->swakelola[$index][$paket_dau->id],
                ]);
                RealisasiDau::whereRaw("periode='$index' AND id_paket_dau='$paket_dau->id' AND tahun='$tahun_anggaran'")->update([
                    'realisasi_keuangan' => $request->realisasi_keuangan[$index][$paket_dau->id],
                    'realisasi_fisik' => $request->realisasi_fisik[$index][$paket_dau->id],
                    'total_keuangan' => $request->tot_res[$paket_dau->id],
                    'total_fisik' => $request->tot_fis[$paket_dau->id],
