...
class LaporanEvaluasiRenjaController extends Controller
{
        ...
        if(!empty($unit_kerja_selected)){
            ...
            if($unit_kerja_selected != ''){
                ...
                return $this->{$fungsi}($tipe, $data, $total_pagu_keseluruhan, $total_realisasi_keuangan, $periode, $dinas, $tahun, $periode_selected, $sumber_dana_selected,$triwulan,$tahun_sebelum);    
                 ...
                 $myData = [];
                 $capaianKinerja = [];
                  $tes = UnitKerja::where(function ($q) {
                            if (hasRole('opd'))
                                $q->where('id', auth()->user()->id_unit_kerja);
                        })->get();
                  
                foreach($tes as $in => $val){
                    $RenstraSubKegiatan = RenstraSubKegiatan::where('id_unit_kerja',$val->id)->get();
                   $myData[$in] = [
                        'nama' => $val->nama_unit_kerja,
                    ];
                  
                    foreach($RenstraSubKegiatan as $kl => $res){
                        // $dpa = Dpa::where('id_sub_kegiatan',$res['id_sub_kegiatan'])->where('tahun',2021)->get();
                       $myData[$in]['sub_kegiatan'][$kl] = $res['id_sub_kegiatan']; 
                    }
                }
                

                foreach($myData as $nmj =>$teet){
                    // return $teet;
                    $capaianKinerja[$nmj] = [
                        'nama_unit_kerja' => $teet['nama']
                    ];
                    if(isset($teet['sub_kegiatan'])){
                        foreach($teet['sub_kegiatan'] as $ghy => $qqw){
                        //    return $qqw;
                            $RenstraSubKegiatanTarget = RenstraSubKegiatanTarget::where('id_renstra_sub_kegiatan',$qqw)->where('tahun',2021)->first();
                            $capaianKinerja[$nmj]['target'][$ghy] = $RenstraSubKegiatanTarget;
                        }
                    }
                }
    ...
    public function export_evaluasi_renja_semua_unit_kerja($tipe, $data, $total_pagu_keseluruhan, $total_realisasi_keuangan, $periode, $dinas, $tahun, $periode_selected, $sumber_dana_selected, $triwulan,$tahun_sebelum, $query = [])
    {
        ...
        // $sheet->getStyle('A' . $cell . ':AA' . $cell)->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB('6D9EEB');
        $cell++;
                    ...
                    foreach ( $urusan->BidangUrusan->sortBy('kode_bidang_urusan') as $bidang_urusan ){
                        ...
                        foreach ( $bidang_urusan->Program->sortBy('kode_program') as $program ){
                            ...
                            foreach ( $program->Kegiatan->sortBy('kode_kegiatan') as $kegiatan ){
                                ...
                                $sheet->setCellValue('Z' . $cell, $sasaran->nama_unit_kerja);
                                $sheet->setCellValue('AA' . $cell, '');
                                $sheet->getStyle('A' . $cell . ':AA' . $cell)->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB('D9D9D9');
                                $cell++;
                                    $n=0;
                                    foreach ( $kegiatan->SubKegiatan->sortBy('kode_sub_kegiatan') as $sub_kegiatan ){
                                        ...
                                        $sheet->setCellValue('G' . $cell, numberToCurrency($sub_kegiatan->total_pagu_renstra));
                                        
                                        $sheet->setCellValue('H' . $cell, $sub_kegiatan->RealisasiK);
                                        $sheet->setCellValue('I' . $cell, numberToCurrency($sub_kegiatan->RealisasiRp));
                                        $sheet->setCellValue('J' . $cell, $sub_kegiatan->TargetK);
                                        $sheet->setCellValue('K' . $cell, numberToCurrency($sub_kegiatan->TargetRp));
                                        $kolom='L';
                                        if ($triwulan==2){
                                        for ($p=1;$p<=$triwulan;$p++){
                                            $sheet->setCellValue($kolom++ . $cell, $sub_kegiatan->RealisasiKinerjaK[$p]);
                                            $sheet->setCellValue($kolom++ . $cell, numberToCurrency($sub_kegiatan->RealisasiKinerjaRp[$p]));
                                        }
                                        $sheet->setCellValue('P' . $cell, '');
                                        $sheet->setCellValue('Q' . $cell, '');
                                        $sheet->setCellValue('R' . $cell, '');
                                        ...
                                        $sheet->setCellValue('U' . $cell, numberToCurrency($sub_kegiatan->Rp13));
                                        $sheet->setCellValue('V' . $cell, $sub_kegiatan->K14);
                                        $sheet->setCellValue('W' . $cell, numberToCurrency($sub_kegiatan->Rp14));
        ...
        $sheet->getStyle('N8:N' . $cell)->getNumberFormat()->setFormatCode('#,##0');
        $sheet->getStyle('O8:O' . $cell)->getNumberFormat()->setFormatCode('#,##0');
        $sheet->getStyle('P8:P' . $cell)->getNumberFormat()->setFormatCode('#,##0');
        $sheet->getStyle('Q8:Q' . $cell)->getNumberFormat()->setFormatCode('#,##0');
        $sheet->getStyle('R8:R' . $cell)->getNumberFormat()->setFormatCode('#,##0');
        $sheet->getStyle('S8:S' . $cell)->getNumberFormat()->setFormatCode('#,##0');
        $sheet->getStyle('T8:T' . $cell)->getNumberFormat()->setFormatCode('#,##0');
        $sheet->getStyle('U8:U' . $cell)->getNumberFormat()->setFormatCode('#,##0');
        $sheet->getStyle('V8:V' . $cell)->getNumberFormat()->setFormatCode('#,##0');
        ...
        // Total
        $cell++;           
        $sheet->setCellValue('A' . $cell, 'Rata-rata capaian kinerja (%)')->mergeCells('A' . $cell . ':O' . $cell);
        ...
        //$sheet->getStyle('A' . ++$cell)->getAlignment()->setHorizontal(\PhpOffice\PhpSpreadsheet\Style\Alignment::HORIZONTAL_LEFT);
    ...
    public function export_evaluasi_renja_semua($tipe, $data, $total_pagu_keseluruhan, $total_realisasi_keuangan, $periode, $dinas, $tahun, $periode_selected, $sumber_dana_selected, $query = [])
    {
        $spreadsheet = new Spreadsheet();
        $spreadsheet->getProperties()->setCreator('AFILA')
            ->setLastModifiedBy('AFILA')
...
//            ->mergeCells('C5:D5');
                            ...
                            //                $sheet->setCellValue('D' . $cell, ($total_pendampingan[$index]));
                                            ...
                                            $sheet->setCellValue('F' . $cell, ($total_kontrak[$index]));
                                            $sheet->setCellValue('E' . $cell, ($total_swakelola[$index]));
                            ...
                            //        $sheet->setCellValue('D' . $cell, ($jumlah_pendampingan));
                                    ...
                                    $sheet->setCellValue('H' . $cell, pembulatanDuaDecimal($persentase_keuangan, 2));
