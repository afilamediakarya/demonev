...
@section('side_form')
    <div id="side_panel" class="offcanvas offcanvas-right p-10">
        ...
        <div class="offcanvas-header d-flex align-items-center justify-content-between pb-7">
            <h4 class="font-weight-bold m-0">Tambah Sasaran</h4>
        ...
        <div class="offcanvas-content">
            <!--begin::Wrapper-->
            <form id="data-form" action="{{route('api.sasaran.create')}}" method="post">
                ...
                <div class="offcanvas-wrapper mb-5 scroll-pull">
                    <div class="form-group">
                        ...
                        <select name="id_tujuan" class="form-control" required></select>
                    </div>
                    <div class="form-group">
                        <label>Sasaran
                            <span class="text-danger">*</span></label>
                        <textarea name="sasaran" class="form-control" id="exampleTextarea" rows="3"
                                  required></textarea>
                    </div>
                    <div class="form-group">
                        <label>Urutan
                            <span class="text-danger">*</span></label>
                        <input name="urutan" type="number" class="form-control" placeholder="Enter Urutan"
                               required/>
                    </div>
                </div>
                <!--end::Wrapper-->
    ...
    <script>
        $(document).ready(function () {
            var getTujuan = function () {
                axios.get('{{route('api.tujuan')}}')
                    .then(function (res) {
                        var tujuan = res.data.response.map(function (val) {
                            return '<option value="' + val.id + '">' + val.tujuan + '</option>'
                        })
                        $('[name=id_tujuan]').append('<option value="" selected disabled>Pilih Tujuan</option>' + tujuan.join(''));
                    })
            }
            getTujuan();
            ...
            var _mainTable = $('.table').DataTable({
                ...
                "ajax": {
                    url: '{{route('api.sasaran.data-table')}}',
                    type: "GET",
            ...
            $('#side_panel_toggle').click(function () {
                $('#side_panel [name]').val('');
                $('[name=id_unit_kerja]').val('{{$id_unit_kerja}}')
            ...
            $('body').on('click', '.open-panel', function (e) {
                ...
                axios.get('{{route('api.sasaran')}}/uuid/' + uuid)
                    .then(function (res) {
            ...
            $('body').on('click', '.button-delete', function (e) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        var uuid = $(this).data('uuid')
                        axios.delete('{{route('api.sasaran')}}/uuid/' + uuid)
                            .then(function () {
                                _mainTable.ajax.reload();
                                Swal.fire('Sukses!', 'Berhasil Menghapus Sasaran', 'success')
                            })
                            .catch(function () {
                    ...
                    if (result.isConfirmed) {
                        ...
                        axios.post(action, $('#data-form').serialize())
                            .then(function () {
                                _mainTable.ajax.reload();
                                _sidePanel.hide();
                                Swal.fire('Sukses!', 'Berhasil Menyimpan Data Sasaran', 'success')
                            })
                            .catch(function (err) {
