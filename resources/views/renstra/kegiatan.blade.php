...
@section('side_form')
    <div id="kt_quick_user" class="offcanvas offcanvas-right p-10">
        ...
        <div class="offcanvas-content">
            <!--begin::Wrapper-->
            <form id="data-form" action="{{route('api.renstra-kegiatan.create')}}" method="post">
                ...
                <div class="offcanvas-wrapper mb-5 scroll-pull">
                    ...
                    <div class="form-group">
                        <label>Program Outcome
                            <span class="text-danger">*</span></label>
                        <select name="id_renstra_program_outcome" id="id_renstra_program_outcome" class="form-control" required>
                            <option value="">Pilih Program Outcome</option>
                        </select>
                    </div>
                    <div class="form-group">
                        <label>Kegiatan
                        ...
                        <select name="id_kegiatan" id="id_kegiatan" class="form-control" required>
                            ...
                            @foreach($kegiatans AS $s)
                                <option value="{{$s->id}}">{{$s->nama_kegiatan}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="row p-0">
                        <div class="col-12 p-0">
                            <div class="row  p-0">
                                <div class="col-12 p-0">
                                    <div id="output" class="card card-custom border-0 card-fit"
                                        style="box-shadow:none">
                                        <div class="card-header border-0 pb-0">
                                            <div class="card-title">
                                                <h3 class="card-label">Output</h3>
                                            </div>
                                            <div class="card-toolbar">
                                                {{-- <a href="javascript:;" data-repeater-create="" id="btn-tambah-output"
                                                class="btn btn-text-dark-50 btn-icon-primary btn-hover-icon-danger font-weight-bold btn-hover-bg-light mr-3">
                                                    <i class="flaticon2-plus-1"></i>Tambah Output</a> --}}
                                            </div>
                                        </div>
...
@push('script')
    <script>
        $(document).ready(function () {
            ...
            $("#id_kegiatan").select2();
            $("#id_program").select2();
            $("#id_renstra_program_outcome").select2();

            var getProgramOutcome = function (program_renstra, _program) {
                $('[name=id_renstra_program_outcome]').removeAttr('disabled');
                $('[name=id_renstra_program_outcome]').html('');
                axios.get('{{route('api.renstra-program')}}/' + program_renstra + '/outcome-renstra-program')
                    .then(function (res) {
                        var program = res.data.response.map(function (val) {
                            return '<option value="' + val.id + '">'+ val.outcome + '</option>'
                        })
                        $('[name=id_renstra_program_outcome]').append('<option value="" selected disabled>Pilih Program Outcome</option>' + program.join(''));
                        if (_program) {
                            $('[name=id_renstra_program_outcome]').val(_program)
                        }
                        $('[name=id_renstra_program_outcome]').select2();
                    })
            }
            
            $('[name=id_program]').change(function () {
                var program_renstra = $(this).val()
                if (program_renstra) {
                    getProgramOutcome(program_renstra, '')
                }
            })
            ...
            var _mainTable = $('.table').DataTable({
                ...
                "ajax": {
                    url: '{{route('api.renstra-kegiatan.data-table')}}',
                    type: "GET",
            ...
            $('#kt_quick_user_toggle').click(function () {
                ...
                $("#id_kegiatan").val('').trigger('change');
                $("#id_program").val('').trigger('change');

                $('[name=id_renstra_program_outcome]').attr('disabled', 'true')
                $("#id_renstra_program_outcome").val('').trigger('change');
                $('[name=id_renstra_program_outcome]').html('')
            ...
            $('body').on('click', '.open-panel', function (e) {
                ...
                $('[data-repeater-item]').slice(1).remove();
                axios.get('{{route('api.renstra-kegiatan')}}/uuid/' + uuid)
                    .then(function (res) {
                        for (key in res.data.response) {
                            ...
                            if(key=='id_kegiatan'){
                                ...
                                $("#id_kegiatan").val(res.data.response[key]).trigger('change');
                            }
                            
                            if(key=='id_renstra_program'){

                                $("#id_program").val(res.data.response[key]);
                            }


                            if(key=='id_renstra_program_outcome'){

                                $("#id_renstra_program_outcome").val(res.data.response[key]).trigger('change');
                            }
                        ...
                        getProgramOutcome(res.data.response.id_renstra_program, res.data.response.id_renstra_program_outcome);
                        
                        res.data.response.renstra_kegiatan_output.forEach(function (val,i){
                            if (i > 0){
                                $('#btn-tambah-output').click();
                            }
                            for (var key in val){
                                if(key=='uuid'){
                                    $('#output [name="['+i+'][out_uuid]"]').val(val[key])
                                }
                                if ($('#output [name="['+i+']['+key+']"]').length)
                                    $('#output [name="['+i+']['+key+']"]').val(val[key])
                            }
                        })
            ...
            $('#output').repeater({
                ...
                show: function () {
                    $(this).slideDown();
                    var index = $('#output [data-repeater-item]').length - 1;
            ...
            $('body').on('click', '.button-delete', function (e) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        var uuid = $(this).data('uuid')
                        axios.delete('{{route('api.renstra-kegiatan')}}/uuid/' + uuid)
                            .then(function () {
                                _mainTable.ajax.reload();
                                Swal.fire('Sukses!', 'Berhasil Menghapus Kegiatan', 'success')
                            })
                            .catch(function () {
            ...
            var fv = FormValidation.formValidation(
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        var el_uuid_pd = $('#output [name*=uuid]');
                        ...
                        var el_output = $('[name*=output]');
                        el_output.each(function (i, el) {
                            data += '&output[' + i + ']=' + $(el).val()
                        })
