                        ...
                        <div class="card card-custom card-stretch example example-compact" id="kt_page_stretched_card">
                            <div class="card-header d-flex flex-column" kt-hidden-height="74" style="">
                                <div class="card-title my-7">
                                    <h3 class="card-label">Laporan DAFTAR ALOKASI</h3>
                                </div>
                                <form id="laporan-form" style="background-color:#f6f9ff" class="py-5 rounded mb-5">
                                    <div class="row">
                                                    ...
                                                    <div class="form-group">
                                                        ...
                                                        <select name="kecamatan" class="form-control">
                                                            ...
                                                            @foreach($kecamatan AS $sd)
                                                                <option
                                                                    value="{{$sd->id}}" {{$kecamatan_selected == $sd->id ? 'selected' : ''}}>{{$sd->nama}}</option>
                                                            @endforeach
                                                        </select>
                                                    </div>
                                                </div>
                                                <div class="col-md col-sm-12">
                                                    <div class="form-group">
                                                        ...
                                                        <select name="desa" class="form-control">
                                                            ...
                                                            <!-- @foreach($desa AS $sd)
                                                                <option
                                                                    value="{{$sd->id}}" {{$desa_selected == $sd->id ? 'selected' : ''}}>{{$sd->nama}}</option>
                                                            @endforeach -->
                                                        </select>
                                                    </div>
                                                </div>
                                                <div class="col-md col-sm-12 hide">
                                                    <div class="form-group">
                                        ...
                                        <div class="col-12 d-flex">
                                            <div class="px-5">
                                                <a id="export-excel" href="{{url('laporan/export_daftar_alokasi/excel')}}" class="btn btn-success btn-xs m-1 ">
                                                    <i class="fas fa-file-excel"></i> Export Excel
                                                </a>
                                                <a id="export-pdf" href="{{url('laporan/export_daftar_alokasi/pdf')}}" class="btn btn-danger btn-xs m-1">
...
@push('script')
    <script>
        let desa = {!! json_encode($desa) !!};
        $('#datepicker').datepicker({
            'format' : 'dd/mm/yyyy'
        });
        $('#export-excel,#export-pdf').click(function(e){
            e.preventDefault();
            let value = $('[name=sumber_dana]').val();

            if (value != null) {
                $('.sumber_dana_error').html('');
                  var url = $(this).attr('href');
                var param = $('#laporan-form').serialize();
                window.open(url+'?'+param, '_blank');
            }else{
                $('.sumber_dana_error').html('Silahkan pilih sumber dana');
            }
        ...
        $('[name=unit_kerja],[name=sumber_dana],[name=periode],[name=desa],[name=kecamatan]').select2();
        ...
        $('[name=kecamatan]').on('change',function () {
            ...
            axios.get('/laporan/desaByKecamatan/' + val)
                .then(function (res) {
                    console.log(res);
                    var option = res.data.map(function (val) {
                        return '<option value="' + val.id + '">'+  val.nama + '</option>'
                    })
                    
                    $('[name=desa]').append('<option value="" >semua</option>' + option.join(''));
        ...
        function selectdesa() {
            var option = desa.map(function (val) {
                            return '<option value="' + val.id + '">'+  val.nama + '</option>'
                        })
                $('[name=desa]').append('<option value="" >semua</option>' + option.join(''));
        ...
        // $.each(kecamatan,function (i,v) {
        //     $('[name=desa]').append(`<option value="" selected disabled>${v->nama}</option>`);
        //     $('[name=desa]').select2();
        // })
