...
@section('side_form')
    <div id="side_panel" class="offcanvas offcanvas-right p-10">
        ...
        <div class="offcanvas-header d-flex align-items-center justify-content-between pb-7">
            <h4 class="font-weight-bold m-0">Tambah Kegiatan Dak</h4>
        ...
        <div class="offcanvas-content">
            <!--begin::Wrapper-->
            <form id="data-form" action="{{route('api.kegiatandak.create')}}" method="post">
                <input type="hidden" name="uuid">
                <input type="hidden" name="tahun" value="">
                <div class="offcanvas-wrapper mb-5 scroll-pull">
                    <div class="form-group">
                        <label for="exampleSelect1">Pilih Sub Bidang Dak
                            <span class="text-danger">*</span></label>
                        <select name="id_sub_bidang" class="form-control" id="exampleSelect1">

                        </select>
                    </div>
                    <div class="form-group">
                        <label>Nama Kegiatan Dak
                            <span class="text-danger">*</span>
                        </label>
                        <textarea name="kegiatan" class="form-control" id="exampleTextarea" rows="3"
                                  required></textarea>
                    </div>
                    <div class="form-group">
                        <label> Kode Kegiatan Dak
                            <span class="text-danger">*</span>
                        </label>
                        <input name="kode_kegiatan" type="text" class="form-control"
                               placeholder="Enter Kode Sub Bidang Dak" required>
                    </div>
                </div>
                <!--end::Wrapper-->
                <!--begin::Purchase-->
    ...
    <script>
        $(document).ready(function () {
            ...
            var _mainTable = $('.table').DataTable({
                ...
                "ajax": {
                    url: '{{route('api.kegiatandak.data-table')}}',
                    type: "GET",
                    headers: {'X-XSRF-TOKEN': Cookies.get("XSRF-TOKEN")},
                },
                columns: [
                    {data: 'kode_kegiatan', name: 'kode_kegiatan'},
                    {data: 'sub_bidang_dak', name: 'sub_bidang_dak', orderable: false, visible: false},
                    {data: 'kegiatan', name: 'kegiatan'},
                    {data: 'action', name: 'action', orderable: false, width: '15%'},
                ],
                rowGroup : {
                    dataSrc : 'sub_bidang_dak'
                }
            });
            axios.get('{{route('api.subbidangdak')}}')
                .then(function (res) {
                    var urusan = res.data.response.map(function (value) {
                        return '<option value="' + value.id + '">' + value.kode_sub_bidang_dak + ' - ' + value.sub_bidang + '</option>'
                    })
                    $('[name=id_sub_bidang]').append('<option value="" selected disabled>Pilih Sub Bidang Dak</option>' + urusan.join(''))
                    $('[name=id_sub_bidang]').select2();
                })
            ...
            $('body').on('click', '.open-panel', function (e) {
                ...
                axios.get('{{route('api.kegiatandak')}}/uuid/' + uuid)
                    .then(function (res) {
                        console.log(res);
            ...
            $('body').on('click', '.button-delete', function (e) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        var uuid = $(this).data('uuid')
                        axios.delete('{{route('api.kegiatandak')}}/uuid/' + uuid)
                            .then(function () {
                                _mainTable.ajax.reload();
                                Swal.fire('Sukses!', 'Berhasil Menghapus Kegiatan Dak', 'success')
                            })
                            .catch(function () {
            ...
            FormValidation.formValidation(
                ...
                Swal.fire({
                    title: 'Perhatian!!',
                    text: state + ' Bidang Kegiatan Dak?',
                    showCancelButton: true,
                    cancelButtonText: 'Batal',
                    ...
                    if (result.isConfirmed) {
                        ...
                        axios.post(action, $('#data-form').serialize())
                            .then(function () {
                                _mainTable.ajax.reload();
                                _sidePanel.hide();
                                Swal.fire('Sukses!', 'Berhasil Menyimpan Data Kegiatan Dak', 'success')
                            })
                            .catch(function (err) {
