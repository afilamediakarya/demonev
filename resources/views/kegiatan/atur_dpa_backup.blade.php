    ...
    <div class="content d-flex flex-column flex-column-fluid" id="kt_content">
        ...
        <div class="content d-flex flex-column flex-column-fluid" id="kt_content">
            <div class="d-flex flex-column-fluid">
                ...
                <div class="container">
                    <!--begin::Card-->
                    <form id="dpa-form" action="{{route('api.dpa.create')}}" method="post">
                        {{csrf_field()}}
                                        ...
                                        <div class="card-body py-0">
                                            <div class="row">
                                                ...
                                                <div class="form-group col-lg-4">
                                                    ...
                                                    <select name="id_kegiatan" id="kegiatan" class="form-control" required
                                                            disabled>
                                                    </select>
                                                </div>
                                                <div class="form-group col-lg-4">
    ...
    <script>
        $(document).ready(function () {
            ...
            var getProgram = function () {
                axios.get('{{route('api.dpa.opsi.program')}}')
                    .then(function (res) {
                        var program = res.data.response.map(function (val) {
                            return '<option value="' + val.id + '">' + val.kode_program + ' - ' + val.nama_program + '</option>'
                        })
                        $('#program').html('<option value="" selected disabled>Pilih Program</option>' + program.join(''))
                        $('#program').select2();
                    })
            }
            var getKegiatan = function (program, _kegiatan, _sub_kegiatan) {
                $('#kegiatan').removeAttr('disabled');
                $('#kegiatan').html('')
                $('#sub_kegiatan').removeAttr('disabled');
                $('#sub_kegiatan').html('')
                axios.get('{{route('api.dpa.opsi.program')}}/' + program + '/kegiatan')
                    .then(function (res) {
                        var kegiatan = res.data.response.map(function (val) {
                            return '<option value="' + val.id + '">' + val.kode_kegiatan + ' - ' + val.nama_kegiatan + '</option>'
                        })
                        $('#kegiatan').append('<option value="" selected disabled>Pilih Kegiatan</option>' + kegiatan.join(''));
                        if (_kegiatan) {
                            $('#kegiatan').val(_kegiatan)
                        }
                        $('#kegiatan').select2();
                    })
            }
            var getSubKegiatan = function (kegiatan, _sub_kegiatan) {
                $('#sub_kegiatan').removeAttr('disabled');
                $('#sub_kegiatan').html('')
                axios.get('{{route('api.dpa.opsi.kegiatan')}}/' + kegiatan + '/sub-kegiatan')
                    .then(function (res) {
                        var sub_kegiatan = res.data.response.map(function (val) {
                            return '<option value="' + val.id + '">' + val.kode_sub_kegiatan + ' - ' + val.nama_sub_kegiatan + '</option>'
                        })
                        $('#sub_kegiatan').append('<option value="" selected disabled>Pilih Sub Kegiatan</option>' + sub_kegiatan.join(''));
                        if (_sub_kegiatan) {
                            $('#sub_kegiatan').val(_sub_kegiatan)
                        }
            ...
            axios.get('{{route('api.dpa.opsi.pj')}}')
                .then(function (res) {
                    var pj = res.data.response.map(function (val) {
                        return '<option value="' + val.id + '">' + val.pegawai.nip + ' - ' + val.pegawai.nama + '</option>'
                    })
            ...
            $('#kegiatan').change(function () {
                getSubKegiatan($(this).val(), '')
            })
            $(document).on('keyup', '[name*="[nilai_pagu]"]', function () {
