...
@section('side_form')
    <div id="side_panel" class="offcanvas offcanvas-right p-10">
        ...
        <div class="offcanvas-header d-flex align-items-center justify-content-between pb-7">
            <h4 class="font-weight-bold m-0">Tambah Role</h4>
        ...
        <div class="offcanvas-content">
            <!--begin::Wrapper-->
            <form id="data-form" action="{{route('api.role.create')}}" method="post">
                <input type="hidden" name="uuid">
                <div class="offcanvas-wrapper mb-5 scroll-pull">
                    <div class="form-group">
                        <label>Nama Role
                            <span class="text-danger">*</span></label>
                        <input name="nama_role" type="text" class="form-control" placeholder="Enter Nama Role"/>
                    </div>
                    <div class="form-group mb-1">
                        <label for="exampleTextarea">Deskripsi Role
                            <span class="text-danger"></span></label>
                        <textarea name="deskripsi" class="form-control" id="exampleTextarea" rows="3"></textarea>
                    </div>
                    <div class="form-group">
                        <label>Pilih Akses</label>
                        <div id="data-akses" class="checkbox-inline">
                        </div>
                    </div>
                </div>
                <!--end::Wrapper-->
        ...
        $(document).ready(function () {
            ...
            var _mainTable = $('.table').DataTable({
                ...
                columns: [
                    ...
                    {data: 'akses_', name: 'akses_', orderable: false},
                    {data: 'action', name: 'action', orderable: false, width: '15%'},
                ],
            });
            axios.get('{{route('api.akses')}}')
                .then(function (res) {
                    var akses = res.data.response.map(function (value) {
                        return '<label class="checkbox checkbox-outline mb-2">' +
                            '<input type="checkbox" value="' + value.id + '" name="akses[' + value.id + ']">' +
                            '<span></span>' + value.nama_akses + '</label>';
                    })
                    $('#data-akses').append(akses.join(''))
                })
            $('#side_panel_toggle').click(function () {
                $(":checkbox").prop('checked', false).parent().removeClass('active');
                $('#side_panel [name]:not([type=checkbox])').val('');
            })
            $('body').on('click', '.open-panel', function (e) {
                ...
                axios.get('{{route('api.role')}}/uuid/' + uuid)
                    .then(function (res) {
                        ...
                        res.data.response.akses.forEach(function (value) {
                            $("[name='akses[" + value.id + "]']").click();
                        })
                    })
                _sidePanel.show();
            })
            $('body').on('click', '.button-delete', function (e) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        var uuid = $(this).data('uuid')
                        axios.delete('{{route('api.role')}}/uuid/' + uuid)
                            .then(function () {
                                _mainTable.ajax.reload();
                                Swal.fire('Sukses!', 'Berhasil Menghapus Role', 'success')
                            })
                            .catch(function () {
                    ...
                    if (result.isConfirmed) {
                        ...
                        axios.post(action, $('#data-form').serialize())
                            .then(function () {
                                _mainTable.ajax.reload();
                                _sidePanel.hide();
                                Swal.fire('Sukses!', 'Berhasil Menyimpan Data Role', 'success')
                            })
                            .catch(function (err) {
